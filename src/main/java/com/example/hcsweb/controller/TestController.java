...
public class TestController {
	...
	public void createPerson() {
		...
		try {
			...
			newPerson.setEffectiveFrom(objectFactory.createResourceEffectiveFrom("0001-01-01T00:00:00")); // default
																											// value
	...
	public void retrieveAgent() {
		...
		try {
			...
			for (Resource res : searchResult) {
				...
				System.out.println(res.getIdentity().getValue() + "\t" + listPair.get(idxLoginName).getValue());
				listAgent.add(new String[] { listPair.get(idxLoginName).getValue(),
						listPair.get(idxLastName).getValue(), listPair.get(idxFirstName).getValue(),

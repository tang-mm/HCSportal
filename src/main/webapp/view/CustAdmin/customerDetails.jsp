			...
			$.ajax({
				...
				success : function(data) {
					...
					$.each(result,
							function(i, elem) {
					var $tr = $('<tr>').append(
							$('<td>').text(elem.tenantName),
							$('<td>').text(elem.ipMain),
							$('<td>').text(elem.description));
					$tr.appendTo('#table-list-tenant');
				});}
			});
			return false;
		});
	...
	<div class="grc-form block" style="margin: 0 auto;">
		<form:form action="customerDetails" id="customerDetailsForm"
			...
			<div class="grc-form">
				<table>
					<tbody>
						<tr class="grc-form-input-text grc-form-no-border">
							...
							<form:input class="modify_toShowUp" path="customerName" value="${customerCmd.customerName}" /></td>
						</tr>
						<tr class="grc-form-no-border">
							...
							<form:textarea class="modify_toShowUp" path="description" value="${customerCmd.description}" rows="5"
									cols="30" /></td>
						</tr>
				...
				<p><h2>Tenants:<%--  ${fn:length(customerCmd.listTenant)} --%></h2></p>
							...
							<c:if test='${not empty customerCmd.listTenant}'>
								<c:forEach items="${customerCmd.listTenant}" var="tenant" varStatus="status">
									<tr>
										...
										<td class="modify_toShowUp"> <a id="<c:out value="deleteTenant_${status.index}"/>" href="removeTenantFromList?name=${tenant.tenantName}" class="deleteRow">Delete</a></td>
									</tr>
								</c:forEach>
							</c:if>
						</tbody>
			...
			<div align="right"><a href="#" class="modify_toHide btn-submit" id="modifyBtn">Modify</a></div>
			</div>
		</form:form>

	<a href="#newTenant" class="modify_toShowUp btn-submit" id="newTenantBtn">Add New Tenant</a>

		<form:form action="addTenantToList?custId=${custId}" id="newTenantForm"

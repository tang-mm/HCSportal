...
$.ajax({
	...
	<div class="grc-form block" style="margin: 0 auto;">
				...
				<table>
					<tbody>
						<tr class="grc-form-input-text grc-form-no-border">
							...
							<td colspan="2"><div class="modify_toHide">${customerCmd.customerName}</div>
								<form:input class="modify_toShowUp" path="customerName"
									value="${customerCmd.customerName}" /></td>
						</tr>
						<tr class="grc-form-no-border">
							<td colspan="1"><label>Description: </label></td>
							<td colspan="2"><div class="modify_toHide">${customerCmd.description}</div>
								<form:textarea class="modify_toShowUp" path="description"
									value="${customerCmd.description}" rows="5" cols="30" /></td>
						</tr>
						<tr class="modify_toShowUp grc-form-buttons grc-form-no-border">
							...
							<td colspan="3"><input type="submit"
								class=" grc-form-buttons-validate" value="Save" name="saveCust"></input></td>
						</tr>
					</tbody>
				</table>
					...
					<table id="table-list-tenant">
						...
						<tbody>
							<c:if test='${not empty customerCmd.listTenant}'>
								<c:forEach items="${customerCmd.listTenant}" var="tenant"
									varStatus="status">
									<tr>
										<td>${tenant.tenantName}</td>
										<td>${tenant.ipMain}</td>
										<td>${tenant.description}</td>
										<td class="modify_toShowUp"><a
											id="<c:out value="deleteTenant_${status.index}"/>"
											href="removeTenantFromList?name=${tenant.tenantName}"
											class="deleteRow">Delete</a></td>
									</tr>
								</c:forEach>
							</c:if>
						</tbody>
		...
		<a href="#newTenant" class="modify_toShowUp btn-submit"
			id="newTenantBtn">Add New Tenant</a>
		<form:form action="addTenantToList?custId=${custId}"
			id="newTenantForm" name="newTenantForm" commandName="newTenantCmd"
			style="display: none;">
			<div class="grc-form">
				<table>
					<tbody>
						...
						<tr class="grc-form-no-border">
							...
							<td colspan="2"><input id="ip1" size="1" maxlength="3"><input
								id="ip2" size="1" maxlength="3"><input id="ip3" size="1"
								maxlength="3" class="ipInput"><input id="ip4" size="1"
								maxlength="3"></td>
							<form:input type="hidden" id="ipFull" path="ipMain"
								maxlength="15" value="" />
						</tr>
						<tr class="grc-form-no-border">
							<td colspan="1"><label>Description: </label></td>
						...
						<tr class="grc-form-buttons grc-form-no-border">
							...
							<td colspan="3"><input type="submit"
								class="grc-form-buttons-validate" value="Add"
